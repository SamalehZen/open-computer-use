...
const data = JSON.parse(fs.readFileSync(path.resolve(__dirname, '..', 'messages', 'de.json'), 'utf8'));
...
for (const [key, budget, policy] of keys) {
  ...
  const ratio = len != null ? +(len / budget).toFixed(2) : null;
  out.push({ key, value, len, budget, ratio, policy });
}
