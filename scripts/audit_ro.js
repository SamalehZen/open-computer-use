const fs = require('fs');
const data = JSON.parse(fs.readFileSync('messages/ro.json', 'utf8'));
...
for (const [key, budget, policy] of keys) {
  ...
  if (val === undefined) {
    results.push({ key, value: '<MISSING>', count: 0, budget, policy, ratio: 0 });
    continue;
  }
  const count = [...String(val)].length;
  results.push({ key, value: val, count, budget, policy, ratio: +(count / budget).toFixed(2) });
}

// Only print rows over budget for focus, plus all wrap-friendly only minor
const violations = results.filter(r => r.count > r.budget);
console.log('=== VIOLATIONS ===');
console.log(JSON.stringify(violations, null, 2));
console.log('=== ALL (count, budget, ratio) ===');
for (const r of results) {
  console.log(`${r.key}\t${r.count}/${r.budget}\t${r.ratio}\t${r.policy}\t${r.value}`);
}
